...
$B.builtin_object_getattro = function(self, klass, attr) {
    ...
    if(in_mro !== $B.NULL &&
            $B.get_class(in_mro) === $B.function &&((! $B.get_dict(self)) || $B.get_from_dict(self, attr, $B.NULL) === $B.NULL)) {
    ...
    if (in_mro !== $B.NULL) {
        ...
        if (getter !== $B.NULL) {
            var is_data_descr = in_mro_class.tp_descr_set !== $B.NULL
            if (is_data_descr) {
                if (test) {
